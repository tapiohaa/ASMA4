...
input_folk = "W:/ASMA4/data/interim/folk_data_2020.csv" 
input_pc_contacts = "W:/ASMA4/data/interim/pc_contacts_public_20" # pc_contacts_public_20XX.rds, XX in 21:22
input_pc_visits = "W:/ASMA4/data/interim/pc_visits_public_20" # pc_visits_public_20.rds, XX in 21:22
input_drugs = "W:/ASMA4/data/interim/prescriptions_" # prescriptions_X.csv, X in 1:3
input_refs = "W:/ASMA4/data/interim/referrals_20" # referrals_20XX.csv, XX in 21:22
...
# Currently, variable letter is %in% c(1:3) for those who received the letter 
# and 0 for those who did not receive the letter. We create a new letter.main 
# variable that instead has letter.main=-1 for those living in a household that 
# received the letter but who did not personally receive the letter.
...
# For 0.07 % of the experimental sample, we cannot find the same person
# in the 2020 FOLK data.
...
presc = lapply(1:3, function(subset) {
  
  # Read visits:
  source = paste(input_drugs, subset, ".csv", sep="")
...
presc = 
  presc[year==2021 & month %in% c(1:9)][, ':=' (month = NULL, year = NULL)]
...
contacts = lapply(study_years, function(yr) {
  ...
  # Aggregate at the ID-date level:
...
sum(contacts$contacts) / (1.33*nrow(ids)) 
...
# Annualized contacts per capita:
sum(visits[contact_type==1, contacts]) / (1.33*nrow(ids))
sum(visits[contact_type==2, contacts]) / (1.33*nrow(ids)) 
...
# Annualized contacts per capita (in 2021):
sum(visits.pri[year==2021, contacts]) / (nrow(ids))
...
presc = lapply(1:3, function(sbst) {
  
  print(sbst)
  
  # Read visits:
  source = paste(input_drugs, sbst, ".csv", sep="")
  df = data.table::fread(source)
  ...
  df[, ':=' (year = as.integer(substr(date_pk, 1, 4)),
             month = as.integer(substr(date_pk, 5, 6)),
             day = as.integer(substr(date_pk, 7, 8)))]
  
  # Aggregate:
  DT = df[, .(contacts = .N), by=c('id', 'year', 'month', 'day', 'sector')]
...
# Aggregate again:
presc = presc[, .(contacts = sum(contacts)), 
...
# Annualized contacts per capita:
sum(presc.pub$contacts) / (1.33*nrow(ids))
...
# Annualized contacts per capita:
sum(presc.pri$contacts) / (1.33*nrow(ids))
...
# All specialized health care contacts contain the arrival date. It is thus
# very likely that the data were extracted requiring that the arrival date
# is observed. Thus, we do not observe those referrals where the patient was
# in a wait list (most patients who got the referral at the end of our 
# follow-up).
...
refs = lapply(study_years, function(year) {
  ...
  df = data.table::fread(source, drop=c('discharged', 'contact_started'))
...
# Origin of the referral is missing in 4% of the cases.
...
100 * colMeans(is.na(refs))


# Extract referrals written by health centers and private outpatient care:
...
# Annualized contacts per capita:
sum(refs[contact_type==4, contacts]) / (1.33*nrow(ids))
sum(refs[contact_type==4.5, contacts]) / (1.33*nrow(ids))
...
# Currently, we have data to the end of 5/2022.
data = data[year==2021 | (year==2022 & month %in% c(1:5))]
