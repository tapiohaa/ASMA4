...
cairo_pdf(filename = output_pc, width = 15.0, height = 12.0)
print(
...
cairo_pdf(filename = output_inc_t1_t23, width = 15.0, height = 12.0)
print(
...
GATES = lapply(results, function(res) {
  
  learners = rownames(res$best$overview)
  
  p = lapply(learners, function(learner) {
    
    # Get the data:
    data = as.data.table(get_GATES(res, learner=learner, plot=FALSE))
    data$X = c(1:5)
    setnames(data, old=c('CB lower', 'CB upper'),
             new=c('conf.low', 'conf.high'))
    
    # Plot:
    
    p = ggplot(data = data, aes(x=X, y=Estimate)) + 
      geom_point() +
      geom_segment(aes(x=X, xend=X, y=conf.low, yend=conf.high)) +
      scale_x_continuous(breaks=c(1:5), 
                         labels=c('G1', 'G2', 'G3', 'G4', 'G4-G1')) +
