...
output_dynamic = "W:/ASMA4/analysis/figures/outcome_trends_dynamic.pdf"
...
# nurse or GP) has been low and stable. We exclude rows where
...
# Only 9% of the actually occurred visits contain data on when the
...
# The rates are the same as with respect to the contact date variable. Our 
# conclusion is to not use triage to primary care as an outcome due to data 
# quality issues.
...
plot.evolution = function(data_visits, data_population, ctype, treat_year) {
  ...
  DT[pc_area=='Eksote', pc_area := 'South Karelia']
  DT[pc_area=='PHHYKY', pc_area := 'Päijät-Häme']
  DT[pc_area=='Kymsote', pc_area := 'Kymenlaakso']
  
  # Start date and end date for the plot:
  
  start_date = as.Date(paste(as.character(treat_year), '-01-01', sep=''))
  end_date = as.Date(paste(as.character(treat_year+1), '-05-01', sep='')) # -06-01 in the PAP, data for 6/22 not yet available
  ...
  p = ggplot(panel, aes(x=date, y=contacts_per_capita, group=pc_area, 
                        shape=pc_area, linetype=pc_area)) +
    geom_point(size=2) +
    geom_line() +
    ggtitle(title) + 
    ...
    labs(linetype='Area', group='Area', shape='Area') +
...
cairo_pdf(filename = output_evol_main, width = 15, height = 12)
...
persons = ids[, id]
contact_types = c(1, 2, 2.5, 3, 3.5, 4, 4.5)
follow_ups = c(0, 1, 2, 3, 4, 5, 6, 7) # c(0:10) in the PAP; we have currently data only to the end of 5/22
...
print(100 * DT.2[, mean(high.user)]) # 4.4% of the individuals are "high.users"
...
### ### ### ### ### ### ### ### ### ### ### ### ### 
#### 9) Accumulation of visits: ID-week. ####
### ### ### ### ### ### ### ### ### ### ### ### ###

# Not pre-registered.


# We will create an ID-week panel in relative time showing the accumulation
# of curative primary care visits (both nurse and GP visits).

# Create a variable for week (in relative time):
DT = df[contact_type %in% c(1, 2), .(id, date, contact_type, contacts)]
DT[, relative_days := date - as.Date('2021-10-13')]
DT[, relative_weeks := as.integer(floor(relative_days / 7))]
DT = DT[relative_weeks %in% c(-40:31)]

# Aggregate at the ID-week level:
DT = DT[, .(contacts = sum(contacts)), 
        by=c('id', 'relative_weeks')]


# Create an empty panel:
data = CJ(id = ids[, id],
          relative_weeks = c(min(DT$relative_weeks) : max(DT$relative_weeks)))

# Merge primary care visits to the panel and impute zeroes if needed.
data = merge(data, DT, by=c('id', 'relative_weeks'), 
             all.x = TRUE)
data[is.na(contacts), contacts := 0]

# Calculate the cumulative sum
csum = data[order(relative_weeks), .(cum_sum = cumsum(contacts)), by=c('id')]
csum[, relative_weeks := rep(c(min(DT$relative_weeks) : max(DT$relative_weeks)),
                             times=nrow(ids))]

# Merge cumulative sums to the data:
data = merge(data, csum, by=c('id', 'relative_weeks'), 
             all.x = TRUE)

# Merge the indicator for high-users:
data = merge(data, DT.2, by='id', all.x = TRUE)

# Merge treatment indicators:
data = merge(data, ids[, .(id, letter.main)], by='id', all.x = TRUE)

# Create a treatment indicator:
data = data[letter.main != -1]
data[letter.main==0, treat := 'No']
data[letter.main %in% c(1:3), treat := 'Yes']


# Aggregate over the sample:
DT = data[, .(cum_sum = mean(cum_sum)), 
          by=c('relative_weeks', 'treat')]

p.1 = ggplot(data=DT, aes(x=relative_weeks, y=cum_sum, group=treat, 
                      linetype=treat)) +
  geom_line() + 
  geom_hline(yintercept = 0, linetype='dashed') + 
  geom_vline(xintercept = 0, linetype='dashed') + 
  xlab('Weeks to the trial') + ylab('Cumulative visits per capita') +
  theme(text = element_text(size=15),     
        axis.text.x = element_text(hjust = 1),
        panel.background = element_rect(fill = "white", colour = "white"),
        panel.grid.major = element_line(size = 0.25, linetype = "solid", 
        ...
        panel.border = element_rect(colour = "black", fill = NA, size = 0.5),
        legend.position = 'bottom') +
  labs(linetype='Received any letter', group='Received any letter')

# Pivot wider:
DT = dcast(DT, relative_weeks ~ 
           paste0('treat_', treat, sep=''), value.var = 'cum_sum')
DT[, difference := treat_Yes - treat_No]
DT[, difference := difference - DT[relative_weeks==-1, difference]]


p.2 = ggplot(data=DT, aes(x=relative_weeks, y=difference)) +
  geom_point() +
  geom_line() + 
  ylim(-0.01, 0.01) +
  geom_hline(yintercept = 0, linetype='dashed') + 
  geom_vline(xintercept = 0, linetype='dashed') +
  xlab('Weeks to the trial') + ylab('Difference in cumulative visits') +
  theme(text = element_text(size=15),     
        axis.text.x = element_text(hjust = 1),
        panel.background = element_rect(fill = "white", colour = "white"),
        panel.grid.major = element_line(size = 0.25, linetype = "solid", 
...
cairo_pdf(filename = output_dynamic, width = 15, height = 8.0)
print(p.1 + p.2 + plot_layout(guides = 'collect') & 
